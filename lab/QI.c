...
// 全局事件队列 (图形引擎将从这里读取本回合发生的所有事情)
#define MAX_EVENTS_PER_TURN 64
static CombatEvent g_event_buffer[MAX_EVENTS_PER_TURN];
...
{
    if (g_event_count >= MAX_EVENTS_PER_TURN)
        return; // 防止缓冲区溢出
    CombatEvent *e = &g_event_buffer[g_event_count++];
    e->event_type = type;
    e->result_type = result;
    e->source_entity_id = src_id;
    e->target_entity_id = tgt_id;
    e->skill_id_used = skill_id;
    e->value = val;
    e->value2 = val2;
}
...
void Initialize_Databases()
{
    ...
    g_skill_database[SKILL_ID_PARRY] = (Skill){SKILL_ID_PARRY, ACTION_TYPE_COUNTER, "格挡", "Parry", 'P', 2, 0, TYPE_PARRY, ATTR_PHYSICAL, 0.5f, 0, 0, TARGET_SELF, "<%s 眼神一凝，摆出了完美的格挡架势。>", "<%s takes a perfect parry stance, ready to counter!>"};
    ...
    g_skill_database[SKILL_ID_BLOOD_DEVIL_DRILL] = (Skill){SKILL_ID_BLOOD_DEVIL_DRILL, ACTION_TYPE_RANGED, "血魔钻", "Blood-Devil Drill", 'R', 9, 3, TYPE_PIERCE, ATTR_BLOOD, 5.0f, 3, 0.5f, TARGET_ENEMY, "<%s 指尖凝聚出一道血色螺旋，化作血魔钻激射而出，直取对手要害！>", "<%s condenses a spiraling bolt of blood energy, unleashing it as a drill that pierces towards the enemy!>"};
    ...
    g_skill_database[SKILL_ID_IMMOVABLE_KING] = (Skill){SKILL_ID_IMMOVABLE_KING, ACTION_TYPE_DEFEND, "不动明王", "Immovable Bright King", 'D', 6, 4, TYPE_RESIST, ATTR_METAL, 0.2f, 3, 1.0f, TARGET_SELF, "<%s 宝相庄严，体表浮现金色梵文，化为不动明王法身，稳如山岳！>", "<%s's body glows with golden sanskrit, forming the Immovable King's aegis, as steadfast as a mountain!>"};
    g_skill_database[SKILL_ID_STELLAR_SHIFT] = (Skill){SKILL_ID_STELLAR_SHIFT, ACTION_TYPE_COUNTER, "斗转星移", "Stellar Shift", 'P', 10, 4, TYPE_REFLECT, ATTR_SPACE, 0.8f, 0, 0, TARGET_SELF, "<%s 身形一晃，引动敌方攻势，周天星斗仿佛随之逆转！>", "<%s's form blurs, redirecting the attack as if shifting the very stars in the cosmos!>"};
    ...
    g_skill_database[SKILL_ID_COSMIC_DHARMA_AVATAR] = (Skill){SKILL_ID_COSMIC_DHARMA_AVATAR, ACTION_TYPE_SMITE, "法相天地", "Cosmic Dharma Avatar", 'S', 50, 4, TYPE_SMASH, ATTR_SPIRITUAL, 40.0f, 0, 0, TARGET_ENEMY, "<%s 身后浮现出通天彻地的法相，一掌拍下，恍若天倾地陷！>", "<%s summons a colossal Dharma Avatar that strikes down with a single palm, like the very heavens collapsing!>"};
    ...
    g_skill_database[SKILL_ID_SPIRIT_SLAYING_SWORD] = (Skill){SKILL_ID_SPIRIT_SLAYING_SWORD, ACTION_TYPE_TERMINATE, "玄天灭灵斩", "Profound Heaven Spirit-Slaying Slash", 'Z', 200, 5, TYPE_SLASH, ATTR_KARMA, 100.0f, 0, 0, TARGET_ENEMY, "<%s 祭出斩灵剑，一道看似朴实无华的剑光斩出，却蕴含着斩断法则的无上之力！>", "<%s wields the Spirit-Slaying Sword, its simple slash containing the supreme power to sever the laws of reality!>"};
}

// --- BLUEPRINT REFACTOR: The New Combat Resolution Engine ---
void Oneway_Solution(Player *attacker, Player *defender)
{
    // --- 准备工作：确定实体ID以供Anima引擎识别 ---
    int attacker_id = (attacker == &YOU) ? ENTITY_ID_YOU : ENTITY_ID_CPU;
    int defender_id = (defender == &YOU) ? ENTITY_ID_YOU : ENTITY_ID_CPU;
    ...
    {
        Record_Event(EVENT_TYPE_DAMAGE, RESULT_TYPE_NORMAL, attacker_id, defender_id, attacker_skill->skill_id, 0, 0);
        return;
    }
    else
    {
    ...
    case TYPE_REFLECT:
        ...
        if (attacker_skill->attribute_id == ATTR_KARMA)
        {
...
CombatEvent* Engine_CalculateNextTurn(ActionType player_action, int* out_event_count)
{
    ...
    // 2. 设置玩家行动
    YOU.current_action_type = player_action;
    YOU.burst_count = 10;
    
    // 模拟原 Player_action 中的消耗逻辑
    // 注意：这里简化处理，假设前端已经验证过QI足够
    if (player_action != ACTION_TYPE_NONE) {
        const Skill *skill = &YOU.learned_skills[player_action];
        YOU.QI_change -= skill->cost;
        
        // 记录玩家的行动宣言事件 (可选，用于显示施法文字)
        // Record_Event(EVENT_TYPE_GAME_STATE, RESULT_TYPE_NORMAL, ENTITY_ID_YOU, -1, skill->skill_id, 0, 0);
    }

    // 3. AI 决策
    CPU.current_action_type = rand() % 10; // 重置
    
    // 模拟原 CPU_action 中的消耗逻辑
    if (CPU.current_action_type != ACTION_TYPE_NONE) {
        const Skill *skill = &CPU.learned_skills[CPU.current_action_type];
        CPU.QI_change -= skill->cost;
    }

    // 4. 核心结算 (这会填充 g_event_buffer)
    // Action_resolve 内部调用了 Oneway_Solution
    Action_resolve(&YOU, &CPU);
    
    // 5. 导出结果
    *out_event_count = g_event_count;
...
void Engine_GetEntitySnapshot(int entity_id, EntitySnapshot* out_snapshot) {
    Player* p = (entity_id == ENTITY_ID_YOU) ? &YOU : &CPU;
    strncpy(out_snapshot->name, p->name, 31);
    out_snapshot->hp = p->HP;
    out_snapshot->max_hp = max_HP[p->XIUWEI];
    out_snapshot->qi = p->QI;
    out_snapshot->max_qi = max_QI[p->XIUWEI];
    out_snapshot->xiuwei_level = p->XIUWEI;
}
